...
<img src="/img/Attention/GraphTransformer.png" alt="GraphTransformer" width="90%" height="auto">
...
**节点特征投影**：$\hat{h}\_{i}^{0} = A^{0}\alpha\_{i} + a^{0}\qquad\qquad\qquad\qquad$ **边特征投影**：$e\_{ij}^{0} = B^{0}\beta\_{ij} + b^{0}$
...
**位置编码投影**$\lambda\_{i}^{0} = C^{0}\lambda\_{i} + c^{0}\qquad\qquad\qquad\qquad$ **节点特征更新**$h\_{i}^{0} = \hat{h}\_{i}^{0} + \lambda\_{i}^{0}$
...
**归一化步骤**$\hat{\hat{h}}\_{i}^{\ell+1} = \text{Norm}(h\_{i}^{\ell} + \hat{h}\_{i}^{\ell+1})\qquad\qquad\qquad$ **前馈网络**$\hat{\hat{\hat{h}}}\_{i}^{\ell+1} = W\_{2}^{\ell} \text{ReLU}(W\_{1}^{\ell} \hat{\hat{h}}\_{i}^{\ell+1})\quad$ 

**最终输出**$h\_{i}^{\ell+1} = \text{Norm}(\hat{\hat{h}}\_{i}^{\ell+1} + \hat{\hat{\hat{h}}}\_{i}^{\ell+1})$
...
**注意力分数计算**$\hat{w}\_{ij}^{k，\ell} = \left( \frac{Q^{k，\ell} h\_{i}^{\ell} \cdot K^{k，\ell} h\_{j}^{\ell}}{\sqrt{d\_{k}}} \right) \cdot E^{k，\ell} e\_{ij}^{\ell}\qquad\qquad\qquad$ **边特征更新**$\hat{e}\_{ij}^{\ell+1} = O\_{e}^{\ell} \prod\_{k=1}^{H} (\hat{w}\_{ij}^{k，\ell})$
...
**最终边输出**$e\_{ij}^{\ell+1} = \text{Norm}(\hat{\hat{e}}\_{ij}^{\ell+1} + \hat{\hat{\hat{e}}}\_{ij}^{\ell+1})$
...
<a href="/paper/Dwivedi和Bresson - 2021 - A Generalization of Transformer Networks to Graphs.pdf" target="_blank">📄 Dwivedi和Bresson - 2021 - A Generalization of Transformer Networks to Graphs</a>

<a href="/paper/Shehzad 等 - 2024 - Graph Transformers A Survey.pdf" target="_blank">📄 Shehzad 等 - 2024 - Graph Transformers A Survey</a>
